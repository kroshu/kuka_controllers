...
controller_interface::CallbackReturn JointGroupPositionController::on_init()
{
  ...
  try
  {
    // Explicitly set the interface parameter declared by the
    // forward_command_controller to match the value set in the
    // JointGroupEffortController constructor.
...
{
  auto per =
    std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::system_clock::now() - t0)
      .count();
  t0 = std::chrono::system_clock::now();
